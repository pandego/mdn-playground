...
def plot_train_val_data(x_train, y_train, x_val, y_val, run_path):
    ...
    fig.update_layout(
        title="Train and Validation Data", xaxis_title="x", yaxis_title="y"
    )
...
def plot_conditional_mode(x_train, y_train, x_test, cond_mode, run_path):
    ...
    fig.add_trace(
        go.Scatter(
            x=x_test.flatten(),
            y=cond_mode.detach().numpy().flatten(),
            mode="markers",
            name="Predictions",
...
def plot_sampled_predictions(x_train, y_train, x_test, preds, run_path):
    ...
    for i in range(preds.shape[1]):
        fig.add_trace(
            go.Scatter(
                x=x_test.flatten(),
                y=preds[:, i, :].detach().numpy().flatten(),
                mode="markers",
...
def plot_histogram(y_pred, y_test, run_path, target_column):
    ...
    hist_pred = go.Histogram(
        x=y_pred_series, nbinsx=nbinsx, name="Predictions", opacity=1
    )
    hist_test = go.Histogram(
        x=y_test_series, nbinsx=nbinsx, name=f"{target_column} Measurements", opacity=1
    )
